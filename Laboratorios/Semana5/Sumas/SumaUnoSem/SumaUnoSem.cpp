 ...
 */
double SumarUnoControlado( Semaforo semid, long * suma ) {
   int i;
 ...
 */
int ForkTestNORaceCondition( int procesos, Semaforo semid, long * total ) {
   int proceso, pid;
...
// Create shared memory segment
   ...
   * total = 0;
   ...
   start = clock();
   * total = 0;
   ForkTestNORaceCondition( procesos, sem, total );
   finish = clock();
   used = ((double) (finish - start)) / CLOCKS_PER_SEC;
